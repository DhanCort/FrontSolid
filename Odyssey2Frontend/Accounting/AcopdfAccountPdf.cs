...
namespace Odyssey2Frontend.Accounting
{
    ...
    public class AcopdfAccountPdf
    {
        ...
        private List<AccountModel> darraccountsmodel = null;
        ...
        //-------------------------------------------------------------------------------------------------------------
        public AcopdfAccountPdf(
            List<AccountModel> darraccountmodel_I,
            List<ReportModel> darrcolJobs_I
            )
        {
            this.darraccountsmodel = darraccountmodel_I;
            ...
            )
        {
            ...
            foreach (AccountModel accountmodel in darraccountsmodel)
            {
                //                                          //Function to add the customer.
                this.subAddJobRow(accountmodel);
            }
        }
            ...
            )
        {
            ...
            for (int intI = 0; intI < this.darrcolJobs.Count; intI++)
            {
                ...
                /*CASE*/
                if (
                    reportmodel.field == "strNumber"
                    )
                {
                    //                                          //First name.
                    paragraph.AddFormattedText(accountmodel_I.strNumber ?? "");
                    paragraph.Format.Font.Size = 8;
                    paragraph.Format.Font.Bold = false;
                }
                else if (
                    reportmodel.field == "strName"
                    )
                {
                    //                                          //Last Name.
                    row.Cells[intI].Format.Alignment = ParagraphAlignment.Left;
                    paragraph.AddFormattedText(accountmodel_I.strName ?? "");
                    paragraph.Format.Font.Size = 7;
                    paragraph.Format.Font.Bold = false;
                }
                else if (
                    reportmodel.field == "numAmount"
                    )
                {
                    //                                          //Last Name.
                    row.Cells[intI].Format.Alignment = ParagraphAlignment.Left;
                    paragraph.AddFormattedText(accountmodel_I.numAmount.ToString() ?? "");
                    paragraph.Format.Font.Size = 7;
                    paragraph.Format.Font.Bold = false;
                }
                else if (
                    reportmodel.field == "strAccountType"
                    )
                {
                    //                                          //Company name.
                    row.Cells[intI].Format.Alignment = ParagraphAlignment.Left;
                    paragraph.AddFormattedText(accountmodel_I.strAccountType ?? "");
                    paragraph.Format.Font.Size = 7;
                    paragraph.Format.Font.Bold = false;
                }
            }
